/**
 * Caminho oficial do piloto real Claude Sonnet 5 Medium vs High.
 *
 * `runExperimentSchedule` permanece genérico: `observeQuota` continua opcional
 * para fixtures. Este módulo é a camada que o piloto real não pode contornar —
 * a Claude quota probe é dependência obrigatória, o adapter Claude
 * REAL_INFERENCE é o executor ligado, e autorização humana nunca é fabricada.
 *
 * Ordem de cada launch, inclusive retry:
 *   authorizeSlot (evidência humana)
 *   → probeClaudeQuota
 *   → deriveQuotaAvailability
 *   → BillingGuard
 *   → ALLOW → executeSlot
 *   INSUFFICIENT/BLOCK → executeSlot não é chamado.
 *
 * A existência deste caminho NÃO autoriza o piloto.
 */
import {
...
/**
 * Snapshot pré-launch: `before_used_pct` é a leitura corrente; `after` permanece
 * ausente. UNAVAILABLE e leitura nula não viram 0% nem SUFFICIENT.
 */
export function quotaUsageFromClaudePreLaunchProbe(outcome: ClaudeQuotaProbeOutcome): QuotaUsage {
  switch (outcome.status) {
    ...
    case QuotaObservationStatus.OBSERVED: {
      if (outcome.reading === null) return unavailableUsage(outcome.provenance);
      return QuotaUsage.parse({
        ...
        windows: [
          ...
          preLaunchWindow(
            ...
            outcome.reading.seven_day_all_models,
            outcome.provenance,
          ),
        ],
      });
...
/** Observer obrigatório do piloto: uma probe Claude nova a cada launch/retry. */
...
/** Confere que todos os arms resolvem o adapter Claude REAL_INFERENCE. */
export function bindClaudePilotExecutor(frozen: FrozenExperimentSpec): ProviderAdapter {
  for (const arm of frozen.spec.arms) {
    ...
    if (adapter.executionKind !== 'REAL_INFERENCE') {
      throw new Error(
        `caminho oficial do piloto exige REAL_INFERENCE; arm "${arm.id}" não qualifica`,
      );
    }
  }
...
/**
 * Monta as dependências oficiais. `observeQuota` não é omitível neste
 * caminho — o scheduler genérico continua aceitando ausência.
 */
export function bindOfficialPilotDependencies(options: {
  ...
  return {
    adapter,
    authorizeSlot: () =>
      decideExecutionAuthorization('REAL_INFERENCE', options.humanAuthorization),
...
/**
 * Inspeção/preflight: lê o spec congelado, liga o adapter Claude e NÃO
 * autoriza, NÃO observa quota de provider e NÃO executa slot.
 */
export async function inspectOfficialPilot(repoRoot: string): Promise<OfficialPilotInspection> {
  ...
  if (adapter.executionKind !== 'REAL_INFERENCE') {
    throw new Error('caminho oficial do piloto exige adapter Claude REAL_INFERENCE');
  }
  return {
...
/**
 * Agenda oficial do piloto. Sempre injeta a Claude quota probe; executeSlot
 * só corre depois de ALLOW. Não inventa autorização humana.
 */
export async function runOfficialPilot(
  options: RunOfficialPilotOptions,
): Promise<RunOfficialPilotResult> {
  const bindings = bindOfficialPilotDependencies({
    frozen: options.frozen,
    quotaProbe: options.quotaProbe,
    ...(options.humanAuthorization === undefined
      ? {}
      : { humanAuthorization: options.humanAuthorization }),
  });
  ...
  const result = await runExperimentSchedule({
    ...
    observeQuota: async (slot) => {
      ...
      quotaObservations.push({
        slot,
        usage,
        availability: derived.availability.value,
      });
      return usage;
    },
