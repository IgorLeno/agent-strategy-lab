 ...
 * e gate explícito para o caminho oficial de execução real (M70).
 *
 * `--dry-run` (ou ausência de `--execute`) lê o `ExperimentSpec`, liga o
 * adapter Claude e NÃO observa quota de provider, NÃO autoriza REAL_INFERENCE
 * e NÃO executa nenhum slot — `authorizes_real_inference` é sempre `false`.
 *
 * `--execute` só avança com `--confirm-real-inference` explícito e um
 * arquivo `--input` com `real_execution_authorization` (evidência humana).
 * Sem esses dois gates nenhum slot real é lançado. A existência deste
 * comando NÃO autoriza o piloto — a decisão ALLOW/BLOCK continua
 * inteiramente em `decideExecutionAuthorization` (`billing/guard.ts`).
 */
...
async function executePilotSlot(slot: PlannedSlot, options: ExecutePilotSlotOptions): Promise<ExecutionStatus> {
