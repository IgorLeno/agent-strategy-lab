#!/usr/bin/env tsx
import { VERBOSE_FLAG, emit, isVerbose, parseArgs, runMain } from '../lib/cli.js';
import { DEFAULT_WORKER_PROFILE_ID } from '../lib/defaults.js';
import { diagnose, doctorFailures, doctorWarnings, summarizeDoctorReport } from '../lib/doctor.js';
 ...
 *
 * Saída padrão: perfil efetivo, warnings e os checks que FALHARAM. `--verbose`
 * acrescenta o relatório inteiro, incluindo os checks que passaram.
 *
...
async function main(): Promise<void> {
  ...
  emit(
    isVerbose(args)
      ? { ...report, warnings: doctorWarnings(report), failures: doctorFailures(report) }
      : summarizeDoctorReport(report),
  );
